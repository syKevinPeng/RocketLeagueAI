...
class PhysicsObject(object):
    def __init__(self, position=None, quaternion=None, linear_velocity=None, angular_velocity=None):
        self.position: np.ndarray = np.asarray(position) if position else np.zeros(3)

        # ones by default to prevent mathematical errors when converting quat to rot matrix on empty physics state
        self.quaternion: np.ndarray = np.asarray(quaternion) if quaternion else np.ones(4)

        self.linear_velocity: np.ndarray = np.asarray(linear_velocity) if linear_velocity else np.zeros(3)
        self.angular_velocity: np.ndarray = np.asarray(angular_velocity) if angular_velocity else np.zeros(3)
